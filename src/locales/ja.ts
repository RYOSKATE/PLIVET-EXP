...
int main()
{
  ...
  message1: String.raw`初めに下記リンク先でPLIVETの使い方をご確認ください。
<a href="https://github.com/RYOSKATE/PLIVET-EXP/wiki/PLIVET使い方" target="_blank">PLIVET使い方</a>  
次に、画面録画アプリ(Bandicam)で録画を開始してください。
録画開始後、このDEMOページでプログラムがどのように実行されるか、
可視化されるかを確認してください。
...
各実験問題は「各自の答え」、「正解・不正解[○×]」、
「回答の所要時間[秒]」が報告対象です。
実験前準備で作成し、開いている実験結果ファイルに記入してください。
EX4の結果記入後は、上部の[回答終了]ボタンを押し、
ログファイルをダウンロード/保存をしてください。`,
  prepare: String.raw`[実験開始]ボタンを押すと問題文とプログラムが表示されます。
自由にプログラムを実行し、可視化結果等を参考にしてください。
問題の答えがわかったら実験記録ファイルに答えをメモし、
[答えを確認]ボタンを押してください。
それぞれのボタンは一度だけしか押せません。
(なのでまだ押さないように注意してください)
[実験開始]ボタンは押すと経過時間が表示されます。
[答えを確認]後は[正解したかどうか]と[経過時間]を
実験結果ファイルに記入し、次の問題に進んでください。
また、この実験ではソースコードの編集はしないでください。
